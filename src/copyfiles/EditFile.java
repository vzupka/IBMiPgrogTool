...
public final class EditFile extends JFrame {
    ...
    boolean textAreaIsSplitV = false;
    boolean textAreaIsSplitH = false;
    ...
    ImageIcon shiftLeftImageIcon;
    ImageIcon shiftRightImageIcon;
    ...
    JButton splitUnsplitButtonV;
    JButton splitUnsplitButtonH;
    JButton findButton;
    ...
    Path splitIconPathV = Paths.get(System.getProperty("user.dir"), "icons", "splitV.png");
    Path splitIconPathH = Paths.get(System.getProperty("user.dir"), "icons", "splitH.png");
    ...
    ShortCaret shortCaret;
    ShortCaret2 shortCaret2;
    ...
    public EditFile(AS400 remoteServer, MainWindow mainWindow,
        ...
        textArea.setLineWrap(false);  // Do not wrap line if the pane is shorter than the line
    ...
    protected void createWindow() {
        ...
        windowWidth = 1050;  // width for font Monospaced size 16
        ...
        // Button "Compile" will be present on editor window only for IBM i, not for PC. 
        if (!methodName.equals("rewritePcFile")) {
        ...
        } else {  // Empty button on editor window for PC. 
            compileButton.setMaximumSize(new Dimension(0, 0));
            compileButton.setMinimumSize(new Dimension(0, 0));
            compileButton.setPreferredSize(new Dimension(0, 0));
        }
        ...
        shiftLeftImageIcon = new ImageIcon(shiftLeftIconPath.toString());
        ...
        // Shift right icon and button
        shiftRightImageIcon = new ImageIcon(shiftRightIconPath.toString());
        ...
        // Split icon and button for vertical split 
        ImageIcon splitImageIconV = new ImageIcon(splitIconPathV.toString());
        splitUnsplitButtonV = new JButton(splitImageIconV);
        splitUnsplitButtonV.setToolTipText("Split/unsplit editor area vertically.");
        splitUnsplitButtonV.setBorder(BorderFactory.createEmptyBorder(4, 4, 4, 4));
        splitUnsplitButtonV.setContentAreaFilled(false);
        splitUnsplitButtonV.setPreferredSize(new Dimension(20, 20));
        splitUnsplitButtonV.setMinimumSize(new Dimension(20, 20));
        splitUnsplitButtonV.setMaximumSize(new Dimension(20, 20));

        // Split icon and button for horizontal split 
        ImageIcon splitImageIconH = new ImageIcon(splitIconPathH.toString());
        splitUnsplitButtonH = new JButton(splitImageIconH);
        splitUnsplitButtonH.setToolTipText("Split/unsplit editor area horizontally.");
        splitUnsplitButtonH.setBorder(BorderFactory.createEmptyBorder(4, 4, 4, 4));
        splitUnsplitButtonH.setContentAreaFilled(false);
        splitUnsplitButtonH.setPreferredSize(new Dimension(20, 20));
        splitUnsplitButtonH.setMinimumSize(new Dimension(20, 20));
        splitUnsplitButtonH.setMaximumSize(new Dimension(20, 20));
        ...
        rowPanel1Layout.setHorizontalGroup(rowPanel1Layout.createSequentialGroup()
                .addGap(0)
                .addComponent(splitUnsplitButtonV)
                .addComponent(splitUnsplitButtonH)
                .addGap(20)
        ...
        rowPanel2Layout.setHorizontalGroup(rowPanel2Layout.createSequentialGroup()
                ...
                .addComponent(saveButton)
                     .addComponent(compileButton)  // Visible for IBM i only
        );
        ...
        // Split/Unsplit vertically button listener
        // ----------------------------------------
        splitUnsplitButtonV.addActionListener(ae -> {
            if (!textAreaIsSplitV) {
                caretPosition = textArea.getCaretPosition();
                textAreaIsSplitV = true;
                textAreaIsSplitH = false;
                unsplitTextArea();
                splitTextArea();
            } else if (textAreaIsSplitV) {
                textAreaIsSplitV = false;
                unsplitTextArea();
            }
        });

        // Split/Unsplit horizontally button listener
        // ----------------------------------------
        splitUnsplitButtonH.addActionListener(ae -> {
            if (!textAreaIsSplitH) {
                caretPosition = textArea.getCaretPosition();
                textAreaIsSplitH = true;
                textAreaIsSplitV = false;
                unsplitTextArea();
                splitTextArea();
            } else if (textAreaIsSplitH) {
                textAreaIsSplitH = false;
                unsplitTextArea();
            }
        });
        ...
        // Find menu item listener (same as Find button)
        menuFind.addActionListener(createFindWindow);
        ...
        // Tab key listener for textArea
        textArea.addKeyListener(new KeyAdapter() {
            @Override
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_TAB) {  // TAB key only
                    e.consume();
                    textArea.insert(fixedLengthSpaces(TAB_SIZE), textArea.getCaretPosition());  // TAB key inserts TAB_SIZE spaces 
                }
                if (e.getKeyCode() == KeyEvent.VK_TAB && e.isShiftDown()) {  // TAB + Shift
                    e.consume();
                    // Text twice tab sizes long before caret position is replaced with empty string (i.e. is deleted). 
                    // One TAB_SIZE is inherited by "textArea.setTabSize(TAB_SIZE);" 
                    textArea.replaceRange("", textArea.getCaretPosition() - TAB_SIZE - TAB_SIZE, textArea.getCaretPosition());
                }
            }
        });
        // Tab key listener for textArea2
        textArea2.addKeyListener(new KeyAdapter() {
            @Override
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_TAB) {  // TAB key only
                    e.consume();
                    textArea2.insert(fixedLengthSpaces(TAB_SIZE), textArea2.getCaretPosition());
                }
                if (e.getKeyCode() == KeyEvent.VK_TAB && e.isShiftDown()) {  // TAB + Shift
                    e.consume();
                    // Text twice tab sizes long before caret position is replaced with empty string (i.e. is deleted). 
                    // One TAB_SIZE is inherited by "textArea.setTabSize(TAB_SIZE);" 
                    textArea2.replaceRange("", textArea2.getCaretPosition() - TAB_SIZE - TAB_SIZE, textArea2.getCaretPosition());
                }
            }
        });
...

        {
            ...
            // Short caret
            shortCaret = new ShortCaret();
            shortCaret2 = new ShortCaret2();
            ...
            if (caretShape.equals(LONG_CARET)) {
                ...
                if (selectionMode.equals(HORIZONTAL_SELECTION)) {
                    ...
                    textArea.setCaret(shortCaret);
                    textArea2.setCaret(shortCaret2);
                } else {
                    // Vertical selection
                    // Set custom caret - long vertical gray line with a short red pointer
                    textArea.setCaret(specialCaret);
    ...
    ***/
        ...
        try {
            ...
            if (selectionModeButton.getText().equals(VERTICAL_SELECTION)) {
                ...
                if (caretButton.getText().equals(SHORT_CARET)) {
                    // Short caret - a short vertical line
                    textArea.setCaret(shortCaret);
                    textArea2.setCaret(shortCaret2);
                } else {  
    ...
    protected void changeCaretShape() {
        try {
            int currentCaretPos = textArea.getCaretPosition();
            int currentCaretPos2 = textArea2.getCaretPosition();
            infile = Files.newBufferedReader(parPath, Charset.forName(encoding));
            properties.load(infile);
            infile.close();
            caretShape = properties.getProperty("CARET");
            ...
            textArea2.setCaretPosition(currentCaretPos2);
    ...
    protected void highlightBlocks(JTextArea textArea) {
        ...
        switch (progLanguage) {
            ...
            case "RPG **FREE", "RPG /FREE" -> {
                ...
                //blkStmts.put("WRITE ", blockBrownLighter);
    ...
    */
    ...
    protected void splitTextArea() {
        ...
        caretPosition = textArea2.getCaretPosition();
        textArea2.requestFocus();

        // Build upper and lower scroll panes, and a split vertical scroll pane
        scrollPaneUpper = new JScrollPane(textArea);
        scrollPaneLower = new JScrollPane(textArea2);

        if (textAreaIsSplitV) {
            splitVerticalPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT);
        } else {
            splitVerticalPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT);
        }
...
/*                        
                        try {
                            tArea.getHighlighter().addHighlight(selectionStart, selectionStart + shiftedText.length(), blockBlueLighter);
                ...
                while (idx < cnt) {
                    ...
                    if (startSel > lineStartOffset) {
                        ...
                        if (! selectedText.isEmpty()) {
                            ...
                            tArea.getHighlighter().addHighlight(startSel - 1, endSel - 1, blockBlueLighter);
...
*/
    ...
    class CustomCopy extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            if (selectionMode.equals(HORIZONTAL_SELECTION)) {
                ...
                } catch (HeadlessException | BadLocationException exc) {
                    exc.printStackTrace();
                }
            } else {
                // Vertical selection
    ...
    class CustomPaste extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            if (selectionMode.equals(HORIZONTAL_SELECTION)) {
                ...
                } catch (UnsupportedFlavorException | IOException | BadLocationException exc) {
                    exc.printStackTrace();
                }
            }
        }
    ...
    public class LongCaret extends DefaultCaret {
        ...
        public void mouseClicked(MouseEvent mouseEvent) {
            ...
            if (mouseEvent.getClickCount() == 2 ) {
                selectionStarts.add(getComponent().getSelectionStart());
                selectionEnds.add(getComponent().getSelectionEnd());
            }
            if (mouseEvent.getClickCount() == 3) {
                selectionStarts.add(getComponent().getSelectionStart());
                selectionEnds.add(getComponent().getSelectionEnd()); 
                // Add 1 to set caret on start of the next line
                getComponent().setSelectionEnd(getComponent().getSelectionEnd() + 1); 
            }
        }
        ...
        public void damage(Rectangle verticalLine) {
            ...
            x = verticalLine.x;
            y = 0;  // from upper end of the area
            height = getComponent().getHeight();  // to the lower end of the area
            width = 2;
        ...
        public void paint(Graphics g) {
            ...
            try {
                verticalLine = component.modelToView2D(dot);
            ...
            if (isVisible()) {
                ...
                g.fillRect((int)verticalLine.getX(), 0, width, getComponent().getHeight());
                // The short line segment of the caret in the y position will be red
                g.setColor(Color.RED);
                g.fillRect((int)verticalLine.getX(), (int)verticalLine.getY(), width, fontSize);
            }
        }
    }
    ...
    /**
     * Implements short caret as a short vertical line with custom thickness
     * for primary text area.
     */
    public class ShortCaret extends DefaultCaret {
        ...
        public void paint(Graphics g) {
            ...
            width = 2;
            g.setColor(Color.RED);
            g.fillRect((int)verticalLine.getX(), (int)verticalLine.getY(), width, fontSize + 2);
        }
    }
     ...
     * for primary text area.
     */
    public class ShortCaret2 extends DefaultCaret {
        ...
        public void paint(Graphics g) {
            ...
            if (isVisible()) {
                ...
                height = fontSize + 2;
                width = 2;
                g.fillRect((int)verticalLine.getX(), (int)verticalLine.getY(), width, height);
            }
        }
    }
    ...
    public class SpecialCaret extends DefaultCaret {
        ...
        protected void moveCaret(MouseEvent mouseEvent) {
            Point2D pt = new Point(mouseEvent.getX(), mouseEvent.getY());
            int pos = getComponent().getUI().viewToModel2D(getComponent(), pt, new Bias[10]);
            if (pos >= 0) {
                setDot(pos);
                Point start = new Point((int)Math.min(lastPoint.x, pt.getX()), (int)Math.min(lastPoint.y, pt.getY()));
                Point end = new Point((int)Math.max(lastPoint.x, pt.getX()), (int)Math.max(lastPoint.y, pt.getY()));
        ...
        protected void customHighlight(Point start, Point end) {
            ...
            int firstX = start.x;
            int lastX = end.x;
            Point2D ptFirst = new Point(firstX, y);
            Point2D ptLast = new Point(lastX, y);
            int pos1 = getComponent().getUI().viewToModel2D(getComponent(), ptFirst, new Bias[10]);
            int pos2 = getComponent().getUI().viewToModel2D(getComponent(), ptLast, new Bias[10]);
            try {
            selectionStarts.add(pos1);
            selectionEnds.add(pos2);
                getComponent().getHighlighter().addHighlight(pos1, pos2, blockBlueLighter);
            } catch (BadLocationException ex) {
                ex.printStackTrace();
            }
            y++;
            while (y < end.y) {                                    
                ptFirst = new Point(firstX, y);
                ptLast = new Point(lastX, y);
                int pos1new = getComponent().getUI().viewToModel2D(getComponent(), ptFirst, new Bias[10]);
                int pos2new = getComponent().getUI().viewToModel2D(getComponent(), ptLast, new Bias[10]);
                if (pos1 != pos1new) {
                    pos1 = pos1new;
                    pos2 = pos2new;
                    try {
