...
This project is not to be installed, it is ready to use as a Java application in the directory of the project. The application requires Java SE 23 or higher installed in PC.
...
- Corrected erroneous shifting block left in vertical selection of text.
- Correction: Enable using a user written pattern for adding names to the user library list.
- Correction: Finding text works in each edited file independently when more files are open for editing.

- - - - - - - - - -

Tento projekt se neinstaluje, je okamžitě k použití jako Java aplikace v adresáři projektu. Aplikace vyžaduje Javu SE 23 instalovanou v PC.
...
- Opraven chybný posun bloku vlevo při vertikálním výběru textu.
- Oprava: Umožní použití ručně zadaného vzorku k přidání jmen do uživatelského seznamu knihoven.
- Oprava: Hledání textu účinkuje v každém editovaném souboru nezávisle, je-li otevřeno více souborů k editaci.
